...
def is_market_open():
    ...
    now = datetime.now(tz)
    # Market open Monday to Friday (0 to 4)
    if now.weekday() > 4:
        return False
...
# Create placeholder container to avoid UI flashing during reruns
...
with main_container.container():
    ...
    if len(df) >= 1:
        ...
        # Main Header Display Status Feed
        st.title(f" {ticker_display} Advanced Real-Time Live Trading Dashboard")
        st.caption(f"Status: {data_status} | Feed Engine Healthy")
        ...
        with kpi4:
            refresh_status = "5 Secs" if is_market_open() else "Paused (Closed)"
            st.markdown(f'<div style="background-color:#1a1a1a; padding:15px; border-radius:8px; border-top:4px solid #00B0FF; text-align:center;"><h5 style="color:#FFFFFF;">Loop Timer</h5><h2 style="color:#00B0FF;">{refresh_status}</h2><span style="color:#FFFFFF;">Auto Refresh Engine</span></div>', unsafe_allow_html=True)
        ...
        with md_col2:
            ...
            if dow_trend == "UPTREND" and live_price > current_vwap and "Long Buildup" in movement_type:
                ...
                risk = entry_exact - stop_loss
                reward = target_exact - entry_exact
                ...
                action_box = f"""<div style="background-color:#0d2e1f; padding:25px; border-radius:12px; border:3px solid #00E676; color:#ffffff;">
                    ...
                    <span style="font-size:16px; color:#FF3D00;"><b>Stop Loss:</b> INR {stop_loss:.2f}</span><br>
                    <span style="font-size:14px; color:#00B0FF;"><b>Estimated Risk-Reward Ratio:</b> 1 : {rr_ratio:.2f}</span>
                </div>"""
            ...
            # SELL SCENARIO
                ...
                # Dynamic Risk Reward Ratio Calculation
                risk = stop_loss - entry_exact
                reward = entry_exact - target_exact
                rr_ratio = (reward / risk) if risk > 0 else 1.5
...
# -----------------------------------------------------------------
# 4. INTELLIGENT STREAMING LIVE RERUN ENGINE
# -----------------------------------------------------------------
# Auto refresh only runs during Market Hours to save system resources
if is_market_open():
    time.sleep(5)
    st.rerun()
else:
