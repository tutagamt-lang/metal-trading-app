...
if len(df) >= 1:
    ...
    # -----------------------------------------------------------------
    # 🎯 REALTIME ADVANCED BREAKOUT SCANNED MATRIX (FIXED VIA COMPONENTS.HTML)
    # -----------------------------------------------------------------
    ...
    if is_near_resistance:
        if day_change > 0 and fut_oi_change_pct > 0:
            ...
            color_box = "#00ff88"
            tamil_desc = "விலை Resistance-ஐ உடைத்து ஏறியுள்ளது. Futures OI மற்றும் பிரைஸ் இரண்டுமே அதிகரிப்பதால் (Long Buildup), Call Writers தங்களின் பொசிஷன்களை மூடிவிட்டு ஓடுகிறார்கள் (Call Unwinding). இது உறுதியான அப்-ட்ரெண்ட்!"
    ...
    # 🌟 பியூர் தனிமைப்படுத்தப்பட்ட சாண்ட்பாக்ஸ் HTML (பாதுகாப்பு அடுக்கைத் தாண்டி வேலை செய்யக்கூடியது)
    scanned_matrix_html = f"""
    <div style="background-color: #0d1117; padding: 20px; border-radius: 6px; border: 1px solid #30363d; border-left: 6px solid {color_box}; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; line-height: 1.8; box-sizing: border-box;">
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #30363d; padding-bottom: 10px; margin-bottom: 15px;">
            <span style="color: {color_box} !important; font-size: 16px; font-family: monospace; font-weight: bold; text-transform: uppercase;">{status_box}</span>
            <span style="font-size: 13px; color: #ffffff !important; font-family: monospace; font-weight: bold;">FUTURES OI CHANGE: <span style="color: #ffcc00 !important;">{fut_oi_change_pct:+.2f}%</span></span>
        </div>
        
        <div style="margin-bottom: 15px; font-size: 14px; color: #ffffff !important;">
            <strong style="color: #ffcc00 !important; font-size: 14px; font-weight: bold;">📊 தமிழ் சந்தை விளக்கம்:</strong> 
            <span style="color: #ffffff !important; font-weight: 500;">{tamil_desc}</span>
        </div>
        
        <div style="background-color: #161b22; padding: 12px 16px; border-radius: 4px; font-size: 14px; border: 1px solid #30363d; color: {color_box} !important; font-weight: bold; letter-spacing: 0.3px;">
            {trade_action}
        </div>
    ...
    # Streamlit பாதுகாப்பு அடுக்கை உடைக்க கஸ்டம் காம்போனென்ட் ஐபிரேம் வழியாக இன்ஜெக்ட் செய்கிறோம்
    components.html(scanned_matrix_html, height=180, scrolling=False)
